...
const defaultOrcoosSymbol = Symbol.for('ondbmongoose:default');
 ...
 *     mongoose instanceof mongoose.Orcoos; // true
 *
 *     // Create a new Orcoos instance with its own `connect()`, `set()`, `model()`, etc.
 *     const m = new mongoose.Orcoos();
 *
 * @api public
 * @param {Object} options see [`Orcoos#set()` docs](/docs/api/mongoose.html#mongoose_Orcoos-set)
 */
function Orcoos(options) {
  this.connections = [];
  ...
  // // If a user creates their own Orcoos instance, give them a separate copy
  // // of the `Schema` constructor so they get separate custom types. (gh-6933)
  // if (!options || !options[defaultOrcoosSymbol]) {
  //   const _this = this;
  ...
  //   // an own property of a Orcoos global, Schema will be undefined. See gh-5648
  //   for (const key of ['Schema', 'model']) {
  //     this[key] = Orcoos.prototype[key];
  //   }
  // }
 ...
 * @api public
 */
Orcoos.prototype.STATES = STATES;
 ...
 * @api public
 */
Orcoos.prototype.ConnectionStates = STATES;

/**
 * Object with `get()` and `set()` containing the underlying driver this Orcoos instance
 * uses to communicate with the database. A driver is a Orcoos-specific interface that defines functions
 * like `find()`.
 *
 * @deprecated
 * @memberOf Orcoos
...
Orcoos.prototype.driver = driver;
...
Orcoos.prototype.setDriver = function setDriver(driver) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
  ...
  if (openConnection) {
    const msg = 'Cannot modify Orcoos driver if a connection is already open. ' +
      'Call `mongoose.disconnect()` before modifying the driver';
    throw new OrcoosError(msg);
  }
 ...
 * - `autoCreate`: Set to `true` to make Orcoos call [`Model.createCollection()`](/docs/api/model.html#model_Model-createCollection) automatically when you create a model with `mongoose.model()` or `conn.model()`. This is useful for testing transactions, change streams, and other features that require the collection to exist.
 * - `autoIndex`: `true` by default. Set to false to disable automatic index creation for all models associated with this Orcoos instance.
 * - `bufferCommands`: enable/disable mongoose's buffering mechanism for all connections and models
 * - `bufferTimeoutMS`: If bufferCommands is on, this option sets the maximum amount of time Orcoos buffering will wait before throwing an error. If not specified, Orcoos will use 10000 (10 seconds).
 * - `cloneSchemas`: `false` by default. Set to `true` to `clone()` all schemas before compiling into a model.
 * - `debug`: If `true`, prints the operations mongoose sends to MongoDB to the console. If a writable stream is passed, it will log to that stream, without colorization. If a callback function is passed, it will receive the collection name, the method name, then all arguments passed to the method. For example, if you wanted to replicate the default logging, you could output from the callback `Orcoos: ${collectionName}.${methodName}(${methodArgs.join(', ')})`.
 ...
 * - `objectIdGetter`: `true` by default. Orcoos adds a getter to MongoDB ObjectId's called `_id` that returns `this` for convenience with populate. Set this to false to remove the getter.
 ...
 * - `sanitizeFilter`: `false` by default. Set to true to enable the [sanitization of the query filters](/docs/api/mongoose.html#mongoose_Orcoos-sanitizeFilter) against query selector injection attacks by wrapping any nested objects that have a property whose name starts with `$` in a `$eq`.
 * - `selectPopulatedPaths`: `true` by default. Set to false to opt out of Orcoos adding all fields that you `populate()` to your `select()`. The schema-level option `selectPopulatedPaths` overwrites this one.
 ...
 * - `toJSON`: `{ transform: true, flattenDecimals: true }` by default. Overwrites default objects to [`toJSON()`](/docs/api/document.html#document_Document-toJSON), for determining how Orcoos documents get serialized by `JSON.stringify()`
 ...
 * @returns {Orcoos} The used Orcoos instnace
 * @api public
 */

Orcoos.prototype.set = function(key, value) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
Orcoos.prototype.get = Orcoos.prototype.set;
 ...
 * @param {Boolean} [options.bufferCommands=true] Orcoos specific option. Set to false to [disable buffering](https://mongoosejs.com/docs/faq.html#callback_never_executes) on all models associated with this connection.
 * @param {String} [options.dbName] The name of the database you want to use. If not provided, Orcoos uses the database name from connection string.
 * @param {String} [options.user] username for authentication, equivalent to `options.auth.user`. Maintained for backwards compatibility.
 * @param {String} [options.pass] password for authentication, equivalent to `options.auth.password`. Maintained for backwards compatibility.
 * @param {Boolean} [options.autoIndex=true] Orcoos-specific option. Set to false to disable automatic index creation for all models associated with this connection.
...
Orcoos.prototype.createConnection = function(uri, options) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
 ...
 * @param {Object} [options] passed down to the [MongoDB driver's `connect()` function](https://mongodb.github.io/node-mongodb-native/4.9/interfaces/MongoClientOptions.html), except for 4 mongoose-specific options explained below.
 * @param {Boolean} [options.bufferCommands=true] Orcoos specific option. Set to false to [disable buffering](https://mongoosejs.com/docs/faq.html#callback_never_executes) on all models associated with this connection.
 * @param {Number} [options.bufferTimeoutMS=10000] Orcoos specific option. If `bufferCommands` is true, Mongoose will throw an error after `bufferTimeoutMS` if the operation is still buffered.
 ...
 * @param {Number} [options.socketTimeoutMS=30000] How long the MongoDB driver will wait before killing a socket due to inactivity _after initial connection_. A socket may be inactive because of either no activity or a long-running operation. This is set to `30000` by default, you should set this to 2-3x your longest running operation if you expect some of your database operations to run longer than 20 seconds. This option is passed to [Node.js `socket#setTimeout()` function](https://nodejs.org/api/net.html#net_socket_settimeout_timeout_callback) after the MongoDB driver successfully completes.
 * @param {Number} [options.family=0] Passed transparently to [Node.js' `dns.lookup()`](https://nodejs.org/api/dns.html#dns_dns_lookup_hostname_options_callback) function. May be either `0`, `4`, or `6`. `4` means use IPv4 only, `6` means use IPv6 only, `0` means try both.
 * @param {Boolean} [options.autoCreate=false] Set to `true` to make Orcoos automatically call `createCollection()` on every model created on this connection.
 * @param {Function} [callback]
 * @see Mongoose#createConnection /docs/api/mongoose.html#mongoose_Orcoos-createConnection
 * @api public
 * @return {Promise} resolves to `this` if connection succeeded
 */

Orcoos.prototype.connect = async function connect(uri, options) {
  if (typeof options === 'function' || (arguments.length >= 3 && typeof arguments[2] === 'function')) {
    throw new OrcoosError('Mongoose.prototype.connect() no longer accepts a callback');
  }
...
Orcoos.prototype.disconnect = async function disconnect() {
  if (arguments.length >= 1 && typeof arguments[0] === 'function') {
    throw new OrcoosError('Orcoos.prototype.disconnect() no longer accepts a callback');
  }
...
Orcoos.prototype.startSession = function() {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
Orcoos.prototype.pluralize = function(fn) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
Orcoos.prototype.model = function(name, schema, collection, options) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
  ...
  if (arguments.length === 1) {
    ...
    if (!model) {
      throw new OrcoosError.MissingSchemaError(name);
    }
    return model;
  }
...
Orcoos.prototype._model = function(name, schema, collection, options) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
Orcoos.prototype.deleteModel = function(name) {
...
Orcoos.prototype.modelNames = function() {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
Orcoos.prototype._applyPlugins = function(schema, options) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
Orcoos.prototype.plugin = function(fn, opts) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
...
/**
 * The Orcoos module's default connection. Equivalent to `mongoose.connections[0]`, see [`connections`](#mongoose_Orcoos-connections).
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
 ...
 *
 * This is the connection used by default for every model created using [mongoose.model](#index_Orcoos-model).
 *
 * To create a new connection, use [`createConnection()`](#mongoose_Orcoos-createConnection).
 *
 * @memberOf Orcoos
 * @instance
...
Orcoos.prototype.__defineGetter__('connection', function() {
  return this.connections[0];
});

Orcoos.prototype.__defineSetter__('connection', function(v) {
 ...
 * Orcoos instance. By default, there is 1 connection. Calling
 * [`createConnection()`](#mongoose_Orcoos-createConnection) adds a connection
 * to this array.
 *
 * #### Example:
 *
...
Orcoos.prototype.connections;

/**
 * An integer containing the value of the next connection id. Calling
 * [`createConnection()`](#mongoose_Orcoos-createConnection) increments
 * this value.
 *
 * #### Example:
 *
...
Orcoos.prototype.nextConnectionId;

/**
 * The Orcoos Aggregate constructor
 *
 * @method Aggregate
 * @api public
 */

Orcoos.prototype.Aggregate = Aggregate;

/**
 * The Orcoos Collection constructor
 *
 * @memberOf Orcoos
 * @instance
 * @method Collection
...
Object.defineProperty(Orcoos.prototype, 'Collection', {
  get: function() {
...
/**
 * The Orcoos [Connection](#connection_Connection) constructor
 *
 * @memberOf Orcoos
 * @instance
 * @method Connection
...
Object.defineProperty(Orcoos.prototype, 'Connection', {
  get: function() {
...
Orcoos.prototype.version = pkg.version;

/**
 * The Orcoos constructor
 *
...
Orcoos.prototype.Mongoose = Orcoos;

/**
 * The Orcoos [Schema](#schema_Schema) constructor
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
...
Orcoos.prototype.Schema = Schema;

/**
 * The Orcoos [SchemaType](#schematype_SchemaType) constructor
 *
 * @method SchemaType
 * @api public
 */

Orcoos.prototype.SchemaType = SchemaType;

/**
 * The various Orcoos SchemaTypes.
 *
 * #### Note:
 *
...
Orcoos.prototype.SchemaTypes = Schema.Types;

/**
 * The Orcoos [VirtualType](#virtualtype_VirtualType) constructor
 *
 * @method VirtualType
 * @api public
 */

Orcoos.prototype.VirtualType = VirtualType;

/**
 * The various Orcoos Types.
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
 ...
 * - [Decimal128](/docs/api/mongoose.html#mongoose_Orcoos-Decimal128)
...
Orcoos.prototype.Types = Types;

/**
 * The Orcoos [Query](#query_Query) constructor.
 *
 * @method Query
 * @api public
 */

Orcoos.prototype.Query = Query;

/**
 * The Orcoos [Model](#model_Model) constructor.
 *
 * @method Model
 * @api public
 */

Orcoos.prototype.Model = Model;

/**
 * The Orcoos [Document](/docs/api/document.html#Document) constructor.
 *
 * @method Document
 * @api public
 */

Orcoos.prototype.Document = Document;

/**
 * The Orcoos DocumentProvider constructor. Orcoos users should not have to
 * use this directly
 *
...
Orcoos.prototype.DocumentProvider = require('./document_provider');

/**
 * The Orcoos ObjectId [SchemaType](/docs/schematypes.html). Used for
...
Orcoos.prototype.ObjectId = SchemaTypes.ObjectId;

/**
 * Returns true if Orcoos can cast the given value to an ObjectId, or
 * false otherwise.
 *
 * #### Example:
 *
 ...
 * @param {Any} v
 * @returns {boolean} true if `v` is something Orcoos can coerce to an ObjectId
 * @api public
 */

Orcoos.prototype.isValidObjectId = function(v) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
  return _mongoose.Types.ObjectId.isValid(v);
};

/**
 * Returns true if the given value is a Orcoos ObjectId (using `instanceof`) or if the
 ...
 * `isValidObjectId()` will return `true` for _any_ value that Orcoos can convert to an
 * ObjectId. That includes Orcoos documents, any string of length 12, and any number.
...
Orcoos.prototype.isObjectIdOrHexString = function(v) {
 ...
 */
Orcoos.prototype.syncIndexes = function(options) {
  const _mongoose = this instanceof Orcoos ? this : mongoose;
  return _mongoose.connection.syncIndexes(options);
};

/**
 * The Orcoos Decimal128 [SchemaType](/docs/schematypes.html). Used for
 * declaring paths in your schema that should be
...
Orcoos.prototype.Decimal128 = SchemaTypes.Decimal128;

/**
 * The Orcoos Mixed [SchemaType](/docs/schematypes.html). Used for
 * declaring paths in your schema that Orcoos's change tracking, casting,
...
Orcoos.prototype.Mixed = SchemaTypes.Mixed;

/**
 * The Orcoos Date [SchemaType](/docs/schematypes.html).
 *
 * #### Example:
 *
...
Orcoos.prototype.Date = SchemaTypes.Date;

/**
 * The Orcoos Number [SchemaType](/docs/schematypes.html). Used for
 * declaring paths in your schema that Orcoos should cast to numbers.
 *
 * #### Example:
 *
...
Orcoos.prototype.Number = SchemaTypes.Number;

/**
 * The [OrcoosError](#error_OrcoosError) constructor.
 *
 * @method Error
 * @api public
 */

Orcoos.prototype.Error = require('./error/index');

/**
 * Orcoos uses this function to get the current time when setting
...
Orcoos.prototype.now = function now() { return new Date(); };

/**
 * The Orcoos CastError constructor
 *
 * @method CastError
...
Orcoos.prototype.CastError = require('./error/cast');
...
Orcoos.prototype.SchemaTypeOptions = require('./options/SchemaTypeOptions');

/**
 * The [node-mongodb-native](https://github.com/mongodb/node-mongodb-native) driver Orcoos uses.
 *
 * @property mongo
 * @api public
 */
...
Orcoos.prototype.mongo = require('oracle-nosqldb');

/**
 * The [mquery](https://github.com/aheckmann/mquery) query builder Orcoos uses.
 *
 * @property mquery
 * @api public
 */

Orcoos.prototype.mquery = require('mquery');
...
Orcoos.prototype.sanitizeFilter = sanitizeFilter;
...
Orcoos.prototype.trusted = trusted;
...
Orcoos.prototype.skipMiddlewareFunction = Kareem.skipWrappedFunction;
...
Orcoos.prototype.overwriteMiddlewareResult = Kareem.overwriteResult;

/**
 * The exports object is an instance of Orcoos.
 *
 * @api private
 */

const mongoose = module.exports = exports = new Orcoos({
  [defaultOrcoosSymbol]: true
});
