...
const defaultOndbMongooseSymbol = Symbol.for('ondbmongoose:default');
...
/**
 * OndbMongoose constructor.
 *
 * The exports object of the `ondbmongoose` module is an instance of this class.
 * Most apps will only use this one instance.
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
 *     mongoose instanceof mongoose.OndbMongoose; // true
 *
 *     // Create a new OndbMongoose instance with its own `connect()`, `set()`, `model()`, etc.
 *     const m = new mongoose.OndbMongoose();
 *
 * @api public
 * @param {Object} options see [`OndbMongoose#set()` docs](/docs/api/mongoose.html#mongoose_OndbMongoose-set)
 */
function OndbMongoose(options) {
  this.connections = [];
  ...
  // // If a user creates their own OndbMongoose instance, give them a separate copy
  // // of the `Schema` constructor so they get separate custom types. (gh-6933)
  // if (!options || !options[defaultOndbMongooseSymbol]) {
  //   const _this = this;
  ...
  //   // an own property of a OndbMongoose global, Schema will be undefined. See gh-5648
  //   for (const key of ['Schema', 'model']) {
  //     this[key] = OndbMongoose.prototype[key];
  //   }
  // }
...
OndbMongoose.prototype.cast = cast;
/**
 * Expose connection states for user-land
 *
 * @memberOf OndbMongoose
 * @property STATES
 * @api public
 */
OndbMongoose.prototype.STATES = STATES;
 ...
 * @api public
 */
OndbMongoose.prototype.ConnectionStates = STATES;

/**
 * Object with `get()` and `set()` containing the underlying driver this OndbMongoose instance
 * uses to communicate with the database. A driver is a OndbMongoose-specific interface that defines functions
 * like `find()`.
 *
 * @deprecated
 * @memberOf OndbMongoose
...
OndbMongoose.prototype.driver = driver;
...
OndbMongoose.prototype.setDriver = function setDriver(driver) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
  ...
  if (openConnection) {
    const msg = 'Cannot modify OndbMongoose driver if a connection is already open. ' +
 ...
 * - `autoCreate`: Set to `true` to make OndbMongoose call [`Model.createCollection()`](/docs/api/model.html#model_Model-createCollection) automatically when you create a model with `mongoose.model()` or `conn.model()`. This is useful for testing transactions, change streams, and other features that require the collection to exist.
 * - `autoIndex`: `true` by default. Set to false to disable automatic index creation for all models associated with this OndbMongoose instance.
 * - `bufferCommands`: enable/disable mongoose's buffering mechanism for all connections and models
 * - `bufferTimeoutMS`: If bufferCommands is on, this option sets the maximum amount of time OndbMongoose buffering will wait before throwing an error. If not specified, OndbMongoose will use 10000 (10 seconds).
 * - `cloneSchemas`: `false` by default. Set to `true` to `clone()` all schemas before compiling into a model.
 * - `debug`: If `true`, prints the operations mongoose sends to MongoDB to the console. If a writable stream is passed, it will log to that stream, without colorization. If a callback function is passed, it will receive the collection name, the method name, then all arguments passed to the method. For example, if you wanted to replicate the default logging, you could output from the callback `OndbMongoose: ${collectionName}.${methodName}(${methodArgs.join(', ')})`.
 ...
 * - `objectIdGetter`: `true` by default. OndbMongoose adds a getter to MongoDB ObjectId's called `_id` that returns `this` for convenience with populate. Set this to false to remove the getter.
 ...
 * - `sanitizeFilter`: `false` by default. Set to true to enable the [sanitization of the query filters](/docs/api/mongoose.html#mongoose_OndbMongoose-sanitizeFilter) against query selector injection attacks by wrapping any nested objects that have a property whose name starts with `$` in a `$eq`.
 * - `selectPopulatedPaths`: `true` by default. Set to false to opt out of OndbMongoose adding all fields that you `populate()` to your `select()`. The schema-level option `selectPopulatedPaths` overwrites this one.
 ...
 * - `toJSON`: `{ transform: true, flattenDecimals: true }` by default. Overwrites default objects to [`toJSON()`](/docs/api/document.html#document_Document-toJSON), for determining how OndbMongoose documents get serialized by `JSON.stringify()`
 ...
 * @returns {OndbMongoose} The used OndbMongoose instnace
 * @api public
 */

OndbMongoose.prototype.set = function(key, value) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
OndbMongoose.prototype.get = OndbMongoose.prototype.set;
 ...
 * @param {Boolean} [options.bufferCommands=true] OndbMongoose specific option. Set to false to [disable buffering](https://mongoosejs.com/docs/faq.html#callback_never_executes) on all models associated with this connection.
 * @param {String} [options.dbName] The name of the database you want to use. If not provided, OndbMongoose uses the database name from connection string.
 * @param {String} [options.user] username for authentication, equivalent to `options.auth.user`. Maintained for backwards compatibility.
 * @param {String} [options.pass] password for authentication, equivalent to `options.auth.password`. Maintained for backwards compatibility.
 * @param {Boolean} [options.autoIndex=true] OndbMongoose-specific option. Set to false to disable automatic index creation for all models associated with this connection.
...
OndbMongoose.prototype.createConnection = function(uri, options) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
 ...
 * @param {Object} [options] passed down to the [MongoDB driver's `connect()` function](https://mongodb.github.io/node-mongodb-native/4.9/interfaces/MongoClientOptions.html), except for 4 mongoose-specific options explained below.
 * @param {Boolean} [options.bufferCommands=true] OndbMongoose specific option. Set to false to [disable buffering](https://mongoosejs.com/docs/faq.html#callback_never_executes) on all models associated with this connection.
 * @param {Number} [options.bufferTimeoutMS=10000] OndbMongoose specific option. If `bufferCommands` is true, Mongoose will throw an error after `bufferTimeoutMS` if the operation is still buffered.
 ...
 * @param {Number} [options.socketTimeoutMS=30000] How long the MongoDB driver will wait before killing a socket due to inactivity _after initial connection_. A socket may be inactive because of either no activity or a long-running operation. This is set to `30000` by default, you should set this to 2-3x your longest running operation if you expect some of your database operations to run longer than 20 seconds. This option is passed to [Node.js `socket#setTimeout()` function](https://nodejs.org/api/net.html#net_socket_settimeout_timeout_callback) after the MongoDB driver successfully completes.
 * @param {Number} [options.family=0] Passed transparently to [Node.js' `dns.lookup()`](https://nodejs.org/api/dns.html#dns_dns_lookup_hostname_options_callback) function. May be either `0`, `4`, or `6`. `4` means use IPv4 only, `6` means use IPv6 only, `0` means try both.
 * @param {Boolean} [options.autoCreate=false] Set to `true` to make OndbMongoose automatically call `createCollection()` on every model created on this connection.
 * @param {Function} [callback]
 * @see Mongoose#createConnection /docs/api/mongoose.html#mongoose_OndbMongoose-createConnection
 * @api public
 * @return {Promise} resolves to `this` if connection succeeded
 */

OndbMongoose.prototype.connect = async function connect(uri, options) {
...
OndbMongoose.prototype.disconnect = async function disconnect() {
  if (arguments.length >= 1 && typeof arguments[0] === 'function') {
    throw new MongooseError('OndbMongoose.prototype.disconnect() no longer accepts a callback');
  }
...
OndbMongoose.prototype.startSession = function() {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
OndbMongoose.prototype.pluralize = function(fn) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
OndbMongoose.prototype.model = function(name, schema, collection, options) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
OndbMongoose.prototype._model = function(name, schema, collection, options) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
OndbMongoose.prototype.deleteModel = function(name) {
...
OndbMongoose.prototype.modelNames = function() {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
OndbMongoose.prototype._applyPlugins = function(schema, options) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
 ...
 * @return {OndbMongoose} this
 * @see plugins /docs/plugins.html
 * @api public
 */

OndbMongoose.prototype.plugin = function(fn, opts) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
...
/**
 * The OndbMongoose module's default connection. Equivalent to `mongoose.connections[0]`, see [`connections`](#mongoose_OndbMongoose-connections).
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
 ...
 *
 * This is the connection used by default for every model created using [mongoose.model](#index_OndbMongoose-model).
 *
 * To create a new connection, use [`createConnection()`](#mongoose_OndbMongoose-createConnection).
 *
 * @memberOf OndbMongoose
 * @instance
...
OndbMongoose.prototype.__defineGetter__('connection', function() {
  return this.connections[0];
});

OndbMongoose.prototype.__defineSetter__('connection', function(v) {
 ...
 * OndbMongoose instance. By default, there is 1 connection. Calling
 * [`createConnection()`](#mongoose_OndbMongoose-createConnection) adds a connection
 * to this array.
 *
 * #### Example:
 *
...
OndbMongoose.prototype.connections;

/**
 * An integer containing the value of the next connection id. Calling
 * [`createConnection()`](#mongoose_OndbMongoose-createConnection) increments
 * this value.
 *
 * #### Example:
 *
...
OndbMongoose.prototype.nextConnectionId;

/**
 * The OndbMongoose Aggregate constructor
 *
 * @method Aggregate
 * @api public
 */

OndbMongoose.prototype.Aggregate = Aggregate;

/**
 * The OndbMongoose Collection constructor
 *
 * @memberOf OndbMongoose
 * @instance
 * @method Collection
...
Object.defineProperty(OndbMongoose.prototype, 'Collection', {
  get: function() {
...
/**
 * The OndbMongoose [Connection](#connection_Connection) constructor
 *
 * @memberOf OndbMongoose
 * @instance
 * @method Connection
...
Object.defineProperty(OndbMongoose.prototype, 'Connection', {
  get: function() {
...
OndbMongoose.prototype.version = pkg.version;

/**
 * The OndbMongoose constructor
 *
 * The exports of the mongoose module is an instance of this class.
 *
 * #### Example:
 *
 *     const mongoose = require('ondbmongoose');
...
OndbMongoose.prototype.Mongoose = OndbMongoose;

/**
 * The OndbMongoose [Schema](#schema_Schema) constructor
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
...
OndbMongoose.prototype.Schema = Schema;

/**
 * The OndbMongoose [SchemaType](#schematype_SchemaType) constructor
 *
 * @method SchemaType
 * @api public
 */

OndbMongoose.prototype.SchemaType = SchemaType;

/**
 * The various OndbMongoose SchemaTypes.
 *
 * #### Note:
 *
...
OndbMongoose.prototype.SchemaTypes = Schema.Types;

/**
 * The OndbMongoose [VirtualType](#virtualtype_VirtualType) constructor
 *
 * @method VirtualType
 * @api public
 */

OndbMongoose.prototype.VirtualType = VirtualType;

/**
 * The various OndbMongoose Types.
 *
 * #### Example:
 *
 *     const mongoose = require('mongoose');
 ...
 * - [Decimal128](/docs/api/mongoose.html#mongoose_OndbMongoose-Decimal128)
...
OndbMongoose.prototype.Types = Types;

/**
 * The OndbMongoose [Query](#query_Query) constructor.
 *
 * @method Query
 * @api public
 */

OndbMongoose.prototype.Query = Query;

/**
 * The OndbMongoose [Model](#model_Model) constructor.
 *
 * @method Model
 * @api public
 */

OndbMongoose.prototype.Model = Model;

/**
 * The OndbMongoose [Document](/docs/api/document.html#Document) constructor.
 *
 * @method Document
 * @api public
 */

OndbMongoose.prototype.Document = Document;

/**
 * The OndbMongoose DocumentProvider constructor. OndbMongoose users should not have to
 * use this directly
 *
...
OndbMongoose.prototype.DocumentProvider = require('./document_provider');

/**
 * The OndbMongoose ObjectId [SchemaType](/docs/schematypes.html). Used for
...
OndbMongoose.prototype.ObjectId = SchemaTypes.ObjectId;

/**
 * Returns true if OndbMongoose can cast the given value to an ObjectId, or
 * false otherwise.
 *
 * #### Example:
 *
 ...
 * @param {Any} v
 * @returns {boolean} true if `v` is something OndbMongoose can coerce to an ObjectId
 * @api public
 */

OndbMongoose.prototype.isValidObjectId = function(v) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
  return _mongoose.Types.ObjectId.isValid(v);
};

/**
 * Returns true if the given value is a OndbMongoose ObjectId (using `instanceof`) or if the
 ...
 * `isValidObjectId()` will return `true` for _any_ value that OndbMongoose can convert to an
 * ObjectId. That includes OndbMongoose documents, any string of length 12, and any number.
...
OndbMongoose.prototype.isObjectIdOrHexString = function(v) {
 ...
 */
OndbMongoose.prototype.syncIndexes = function(options) {
  const _mongoose = this instanceof OndbMongoose ? this : mongoose;
  return _mongoose.connection.syncIndexes(options);
};

/**
 * The OndbMongoose Decimal128 [SchemaType](/docs/schematypes.html). Used for
 * declaring paths in your schema that should be
...
OndbMongoose.prototype.Decimal128 = SchemaTypes.Decimal128;

/**
 * The OndbMongoose Mixed [SchemaType](/docs/schematypes.html). Used for
 * declaring paths in your schema that OndbMongoose's change tracking, casting,
...
OndbMongoose.prototype.Mixed = SchemaTypes.Mixed;

/**
 * The OndbMongoose Date [SchemaType](/docs/schematypes.html).
 *
 * #### Example:
 *
...
OndbMongoose.prototype.Date = SchemaTypes.Date;

/**
 * The OndbMongoose Number [SchemaType](/docs/schematypes.html). Used for
 * declaring paths in your schema that OndbMongoose should cast to numbers.
 *
 * #### Example:
 *
...
OndbMongoose.prototype.Number = SchemaTypes.Number;

/**
 * The [MongooseError](#error_OndbMongooseError) constructor.
 *
 * @method Error
 * @api public
 */

OndbMongoose.prototype.Error = require('./error/index');

/**
 * OndbMongoose uses this function to get the current time when setting
...
OndbMongoose.prototype.now = function now() { return new Date(); };

/**
 * The OndbMongoose CastError constructor
 *
 * @method CastError
...
OndbMongoose.prototype.CastError = require('./error/cast');
...
OndbMongoose.prototype.SchemaTypeOptions = require('./options/SchemaTypeOptions');

/**
 * The [node-mongodb-native](https://github.com/mongodb/node-mongodb-native) driver OndbMongoose uses.
 *
 * @property mongo
 * @api public
 */
...
OndbMongoose.prototype.mongo = require('oracle-nosqldb');

/**
 * The [mquery](https://github.com/aheckmann/mquery) query builder OndbMongoose uses.
 *
 * @property mquery
 * @api public
 */

OndbMongoose.prototype.mquery = require('mquery');
...
OndbMongoose.prototype.sanitizeFilter = sanitizeFilter;
...
OndbMongoose.prototype.trusted = trusted;
...
OndbMongoose.prototype.skipMiddlewareFunction = Kareem.skipWrappedFunction;
...
OndbMongoose.prototype.overwriteMiddlewareResult = Kareem.overwriteResult;

/**
 * The exports object is an instance of OndbMongoose.
 *
 * @api private
 */

const mongoose = module.exports = exports = new OndbMongoose({
  [defaultOndbMongooseSymbol]: true
});
