...
const defaultConnectionSymbol = Symbol('orcoos:defaultConnection');
 ...
 *     orcoos instanceof orcoos.Mongoose; // true
 *
 *     // Create a new Mongoose instance with its own `connect()`, `set()`, `model()`, etc.
 *     const m = new orcoos.Mongoose();
 *
 * @api public
 * @param {Object} options see [`Mongoose#set()` docs](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.set())
 */
function Mongoose(options) {
  ...
  if (!options || !options[defaultMongooseSymbol]) {
    ...
    // `import orcoos, { Schema } from 'orcoos'`. Because `Schema` is not
...
Mongoose.prototype.setDriver = function setDriver(driver) {
  const _mongoose = this instanceof Mongoose ? this : orcoos;
  ...
  if (openConnection) {
    const msg = 'Cannot modify Mongoose driver if a connection is already open. ' +
      'Call `orcoos.disconnect()` before modifying the driver';
 ...
 * #### Example:
 *
 *     orcoos.set('test', value) // sets the 'test' option to `value`
 *
 *     orcoos.set('debug', true) // enable logging collection methods + arguments to the console/file
 *
 *     orcoos.set('debug', function(collectionName, methodName, ...methodArgs) {}); // use custom function to log collection methods + arguments
 *
 *     orcoos.set({ debug: true, autoIndex: false }); // set multiple options at once
 *
 ...
 * - `autoCreate`: Set to `true` to make Mongoose call [`Model.createCollection()`](https://mongoosejs.com/docs/api/model.html#Model.createCollection()) automatically when you create a model with `orcoos.model()` or `conn.model()`. This is useful for testing transactions, change streams, and other features that require the collection to exist.
 * - `autoIndex`: `true` by default. Set to false to disable automatic index creation for all models associated with this Mongoose instance.
 * - `bufferCommands`: enable/disable orcoos's buffering mechanism for all connections and models
 ...
 * - `debug`: If `true`, prints the operations orcoos sends to MongoDB to the console. If a writable stream is passed, it will log to that stream, without colorization. If a callback function is passed, it will receive the collection name, the method name, then all arguments passed to the method. For example, if you wanted to replicate the default logging, you could output from the callback `Mongoose: ${collectionName}.${methodName}(${methodArgs.join(', ')})`.
 ...
 * - `overwriteModels`: Set to `true` to default to overwriting models with the same name when calling `orcoos.model()`, as opposed to throwing an `OverwriteModelError`.
 ...
 * - `sanitizeFilter`: `false` by default. Set to true to enable the [sanitization of the query filters](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.sanitizeFilter()) against query selector injection attacks by wrapping any nested objects that have a property whose name starts with `$` in a `$eq`.
 ...
 * #### Example:
 *
 *     orcoos.get('test') // returns the 'test' value
 *
 * @param {String} key
 * @method get
 * @api public
 ...
 *
 *     // with mongodb:// URI
 *     db = orcoos.createConnection('mongodb://user:pass@127.0.0.1:port/database');
 *
 *     // and options
 *     const opts = { db: { native_parser: true }}
 *     db = orcoos.createConnection('mongodb://user:pass@127.0.0.1:port/database', opts);
 *
 *     // replica sets
 *     db = orcoos.createConnection('mongodb://user:pass@127.0.0.1:port,anotherhost:port,yetanother:port/database');
 *
 *     // and options
 *     const opts = { replset: { strategy: 'ping', rs_name: 'testSet' }}
 *     db = orcoos.createConnection('mongodb://user:pass@127.0.0.1:port,anotherhost:port,yetanother:port/database', opts);
 *
 *     // initialize now, connect later
 *     db = orcoos.createConnection();
 *     await db.openUri('mongodb://127.0.0.1:27017/database');
 *
 * @param {String} uri mongodb URI to connect to
 * @param {Object} [options] passed down to the [MongoDB driver's `connect()` function](https://mongodb.github.io/node-mongodb-native/4.9/interfaces/MongoClientOptions.html), except for 4 orcoos-specific options explained below.
 ...
 * @return {Connection} the created Connection object. Connections are not thenable, so you can't do `await orcoos.createConnection()`. To await use `orcoos.createConnection(uri).asPromise()` instead.
 * @api public
 */
...
/**
 * Opens the default orcoos connection.
 *
 * #### Example:
 *
 *     orcoos.connect('mongodb://user:pass@127.0.0.1:port/database');
 *
 *     // replica sets
 ...
 * @see Mongoose#createConnection https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.createConnection()
 * @api public
 ...
 *
 * Calling `orcoos.startSession()` is equivalent to calling `orcoos.connection.startSession()`.
 * Sessions are scoped to a connection, so calling `orcoos.startSession()`
 * starts a session on the [default orcoos connection](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.connection).
 *
 ...
 * @return {Function|null} the current function used to pluralize collection names, defaults to the legacy function from `orcoos-legacy-pluralize`.
 * @api public
 */
 ...
 *
 * Models defined on the `orcoos` instance are available to all connection
 * created by the same `orcoos` instance.
 *
 * If you call `orcoos.model()` with twice the same name but a different schema,
 * you will get an `OverwriteModelError`. If you call `orcoos.model()` with
 ...
 *     const orcoos = require('orcoos');
 *
 *     // define an Actor model with this orcoos instance
 *     const schema = new Schema({ name: String });
 *     orcoos.model('Actor', schema);
 *
 *     // create a new connection
 *     const conn = orcoos.createConnection(..);
 *
 *     // create Actor model
 ...
 *
 * _When no `collection` argument is passed, Mongoose uses the model name. If you don't like this behavior, either pass a collection name, use `orcoos.pluralize()`, or set your schemas collection name option._
 *
 * #### Example:
 *
 ...
 *     const M = orcoos.model('Actor', schema, collectionName);
 *
...
Mongoose.prototype.model = function(name, schema, collection, options) {
  ...
  if (schema && !(schema instanceof Schema)) {
    throw new Error('The 2nd parameter to `orcoos.model()` should be a ' +
      'schema or a POJO');
  }
 ...
 *
 * Equivalent to `orcoos.connection.deleteModel(name)`.
 *
 * #### Example:
 *
 *     orcoos.model('User', new Schema({ name: String }));
 *     console.log(orcoos.model('User')); // Model object
 *     orcoos.deleteModel('User');
 *     console.log(orcoos.model('User')); // undefined
 *
 *     // Usually useful in a Mocha `afterEach()` hook
 *     afterEach(function() {
 *       orcoos.deleteModel(/.+/); // Delete every model
 *     });
 *
 * @api public
...
/**
 * The Mongoose module's default connection. Equivalent to `orcoos.connections[0]`, see [`connections`](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.connections).
 *
 * #### Example:
 *
 *     const orcoos = require('orcoos');
 *     orcoos.connect(...);
 *     orcoos.connection.on('error', cb);
 *
 * This is the connection used by default for every model created using [orcoos.model](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.model()).
 *
 * To create a new connection, use [`createConnection()`](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.createConnection()).
 *
 * @memberOf Mongoose
 * @instance
 ...
 * [`createConnection()`](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.createConnection()) adds a connection
 * to this array.
 *
 * #### Example:
 *
 *     const orcoos = require('orcoos');
 *     orcoos.connections.length; // 1, just the default connection
 *     orcoos.connections[0] === orcoos.connection; // true
 *
 *     orcoos.createConnection('mongodb://127.0.0.1:27017/test');
 *     orcoos.connections.length; // 2
 *
 * @memberOf Mongoose
 * @instance
 ...
 * [`createConnection()`](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.createConnection()) increments
 * this value.
 *
 * #### Example:
 *
 *     const orcoos = require('orcoos');
 *     orcoos.createConnection(); // id `0`, `nextConnectionId` becomes `1`
 *     orcoos.createConnection(); // id `1`, `nextConnectionId` becomes `2`
 *     orcoos.connections[0].destroy() // Removes connection with id `0`
 *     orcoos.createConnection(); // id `2`, `nextConnectionId` becomes `3`
 *
 * @memberOf Mongoose
 * @instance
 ...
 * #### Example:
 *
 *     console.log(orcoos.version); // '5.x.x'
 *
 * @property version
 * @api public
 */
 ...
 *
 * The exports of the orcoos module is an instance of this class.
 *
 * #### Example:
 *
 *     const orcoos = require('orcoos');
 *     const mongoose2 = new orcoos.Mongoose();
 *
 * @method Mongoose
 * @api public
 */
 ...
 * #### Note:
 *
 * _Alias of orcoos.Schema.Types for backwards compatibility._
 *
 ...
 *
 *     const orcoos = require('orcoos');
 *     const array = orcoos.Types.Array;
 *
 * #### Types:
 *
 ...
 * - [Decimal128](https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.Decimal128)
 ...
 *
 *     const ObjectId = orcoos.Types.ObjectId;
 ...
 * Do not use this to create a new ObjectId instance, use `orcoos.Types.ObjectId`
 * instead.
 *
 * #### Example:
 *
 *     const childSchema = new Schema({ parentId: orcoos.ObjectId });
 *
 * @property ObjectId
 * @api public
 */
 ...
 * #### Example:
 *
 *     orcoos.isValidObjectId(new orcoos.Types.ObjectId()); // true
 *     orcoos.isValidObjectId('0123456789ab'); // true
 *     orcoos.isValidObjectId(6); // true
 *     orcoos.isValidObjectId(new User({ name: 'test' })); // true
 *
 *     orcoos.isValidObjectId({ test: 42 }); // false
 *
 ...
 * #### Example:
 *
 *     orcoos.isObjectIdOrHexString(new orcoos.Types.ObjectId()); // true
 *     orcoos.isObjectIdOrHexString('62261a65d66c6be0a63c051f'); // true
 *
 *     orcoos.isObjectIdOrHexString('0123456789ab'); // false
 *     orcoos.isObjectIdOrHexString(6); // false
 *     orcoos.isObjectIdOrHexString(new User({ name: 'test' })); // false
 *     orcoos.isObjectIdOrHexString({ test: 42 }); // false
 *
 ...
 *
 * @param {Object} options
 * @param {Boolean} options.continueOnError `false` by default. If set to `true`, orcoos will not throw an error if one model syncing failed, and will return an object where the keys are the names of the models, and the values are the results/errors for each model.
 ...
 * Do not use this to create a new Decimal128 instance, use `orcoos.Types.Decimal128`
 * instead.
 *
 * #### Example:
 *
 *     const vehicleSchema = new Schema({ fuelLevel: orcoos.Decimal128 });
 *
 * @property Decimal128
 * @api public
 */
 ...
 * #### Example:
 *
 *     const schema = new Schema({ arbitrary: orcoos.Mixed });
 *
 * @property Mixed
 * @api public
 */
 ...
 *     schema.path('test') instanceof orcoos.Date; // true
 *
 * @property Date
 * @api public
 */
 ...
 * #### Example:
 *
 *     const schema = new Schema({ num: orcoos.Number });
 *     // Equivalent to:
 ...
 *       return orcoos.skipMiddlewareFunction({ matchedCount: 0 });
 *     });
 *
 ...
 *       return orcoos.overwriteMiddlewareResult(res.filter(doc => !doc.isDeleted));
 *     });
 *
 ...
 *     orcoos.omitUndefined(filter); // { name: 'John', status: 'active' }
 *     filter; // { name: 'John', status: 'active' }
 *
 *     await UserModel.findOne(orcoos.omitUndefined(filter));
 *
...
/*!
 * Create a new default connection (`orcoos.connection`) for a Mongoose instance.
 * No-op if there is already a default connection.
 */

function _createDefaultConnection(orcoos) {
  if (orcoos.connection) {
    return;
  }
  const conn = orcoos.createConnection(); // default connection
  conn[defaultConnectionSymbol] = true;
  conn.models = orcoos.models;
}
...
const orcoos = module.exports = exports = new Mongoose({
