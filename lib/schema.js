...
Schema.prototype.pick = function(paths, options) {
  ...
  if (!Array.isArray(paths)) {
    throw new OrcoosError('Schema#pick() only accepts an array argument, ' +
      'got "' + typeof paths + '"');
  }
  ...
  for (const path of paths) {
    if (this.nested[path]) {
      ...
      if (schematype == null) {
        throw new OrcoosError('Path `' + path + '` is not in the schema');
      }
...
Schema.prototype.defaultOptions = function(options) {
  ...
  if (options.versionKey && typeof options.versionKey !== 'string') {
    throw new OrcoosError('`versionKey` must be falsy or string, got `' + (typeof options.versionKey) + '`');
  }
  ...
  if (options.optimisticConcurrency && !options.versionKey) {
    throw new OrcoosError('Must set `versionKey` if using `optimisticConcurrency`');
  }
    ...
    if (_mapType[schema.options.typeKey] && _mapType[schema.options.typeKey].instanceOfSchema) {
      ...
      subdocumentSchema.eachPath((subpath, type) => {
        if (type.options.select === true || type.options.select === false) {
          throw new OrcoosError('Cannot use schema-level projections (`select: true` or `select: false`) within maps at path "' + path + '.' + subpath + '"');
        }
      });
    }
  ...
  if (Array.isArray(path)) {
    for (const virtual of path) {
      if (this.virtuals[virtual] == null) {
        throw new OrcoosError(`Attempting to remove virtual "${virtual}" that does not exist.`);
      }
    }
