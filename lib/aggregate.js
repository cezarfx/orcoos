...
Aggregate.prototype.explain = async function explain(verbosity) {
  if (typeof verbosity === 'function' || typeof arguments[1] === 'function') {
    throw new OrcoosError('Aggregate.prototype.explain() no longer accepts a callback');
  }
...
Aggregate.prototype.exec = async function exec() {
  ...
  if (typeof arguments[0] === 'function') {
    throw new OrcoosError('Aggregate.prototype.exec() no longer accepts a callback');
  }
  const model = this._model;
  ...
  if (!this._pipeline.length) {
    throw new OrcoosError('Aggregate has empty pipeline');
  }
