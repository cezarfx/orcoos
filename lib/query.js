 ...
 *
 * ```javascript
 * orcoos.Query.use$geoWithin = false;
 * ```
 *
 ...
 * #### Note:
 *
 * In Mongoose 3.7, `within` changed from a getter to a function. If you need the old syntax, use [this](https://github.com/ebensing/orcoos-within).
 *
 * @method within
 ...
 *     });
 *     const UserModel = orcoos.model('User', userSchema);
 ...
 * @param {Boolean} value
 * @return {Query} this
 * @see sanitizeProjection https://thecodebarbarian.com/whats-new-in-orcoos-5-13-sanitizeprojection.html
 * @api public
 */
 ...
 * #### Example:
 *
 *     const s = await orcoos.startSession();
 *     await orcoos.model('Person').findOne({ name: 'Axl Rose' }).session(s);
 *
 * @method session
 * @memberOf Query
 * @instance
 ...
 * @see orcoos.startSession() https://mongoosejs.com/docs/api/orcoos.html#Mongoose.prototype.startSession()
 * @return {Query} this
 * @api public
 */
 ...
 * #### Example:
 *
 *     await orcoos.model('Person').deleteOne({ name: 'Ned Stark' }).j(true);
 *
 * @method j
 * @memberOf Query
 * @instance
 ...
 *     docs[0] instanceof orcoos.Document; // false
 *
 ...
 * to use a plugin. See:
 *
 * - [orcoos-lean-virtuals](https://plugins.mongoosejs.io/plugins/lean-virtuals)
 * - [orcoos-lean-getters](https://plugins.mongoosejs.io/plugins/lean-getters)
 * - [orcoos-lean-defaults](https://www.npmjs.com/package/orcoos-lean-defaults)
 *
...
/**
 * Getter/setter around the current orcoos-specific options for this query
...
Query.prototype._find = async function _find() {
  ...
  const cursor = await this.mongooseCollection.find(filter, options, this.schema);
  if (options.explain) {
...
/**
 * Execute NoSQL DB SQL query. The result will be an array of documents.
 *
 *
 * #### Example:
 *
 *     const arr = await Movie.find("select * from Movie t where t.kvjson.year >= 1980 and t.kvjson.year <= 1989");
 *
 * @param {String} [statement] NoSQL DB SQL statement. If not specified, returns all documents.
 * @return {Query} this
 * @api public
 */

Query.prototype.nosqlQuery = function(statement, options) {
  this.op = 'nosqlQuery';
  this._statement = statement;
  this._nosqlQueryOptions = options;
  if (Query.base && Query.base.nosqlQuery)
    Query.base.nosqlQuery.call(this, statement, options);

  return this;
};

/**
 * Execute a native NoSQL DB query.
 *
 * @return {Query} this
 * @api private
 */
Query.prototype._nosqlQuery = async function _nosqlQuery() {
  this._applyTranslateAliases();
  this._castConditions();
  ...
  const options = this._optionsForExec();
  if (utils.hasUserDefinedProperty(this._nosqlQueryOptions, 'bindings')) {
    options['bindings'] = this._nosqlQueryOptions['bindings'];
  }

  const filter = this._conditions;
  const fields = options.projection;

  const cursor = await this.mongooseCollection.nosqlQuery(statement, options);
  if (options.explain) {
    return cursor.explain();
  }
 ...
 * orcoos will send an empty `findOne` command to MongoDB, which will return
 ...
 * - `setDefaultsOnInsert`: `true` by default. If `setDefaultsOnInsert` and `upsert` are true, orcoos will apply the [defaults](https://mongoosejs.com/docs/defaults.html) specified in the model's schema if a new document is created.
 *
 * #### Example:
 *
 ...
 * @param {Boolean} [options.multipleCastError] by default, orcoos only returns the first error that occurred in casting the query. Turn on this option to aggregate all the cast errors.
 ...
 * @param {Object} [options.lean] if truthy, orcoos will return the document as a plain JavaScript object rather than a orcoos document. See [`Query.lean()`](https://mongoosejs.com/docs/api/query.html#Query.prototype.lean()) and [the Mongoose lean tutorial](https://mongoosejs.com/docs/tutorials/lean.html).
 * @param {ClientSession} [options.session=null] The session associated with this query. See [transactions docs](https://mongoosejs.com/docs/transactions.html).
 * @param {Boolean|String} [options.strict] overwrites the schema's [strict mode option](https://mongoosejs.com/docs/guide.html#strict)
...
/**
 * Get options from query opts, falling back to the base orcoos object.
 * @param {Query} query
 * @param {Object} option
 * @param {Any} def
 * @api private
...
/**
 * Override mquery.prototype._mergeUpdate to handle orcoos objects in
 * updates.
 *
 * @param {Object} doc
 * @method _mergeUpdate
...
Query.prototype.exec = async function exec(op) {
  ...
  try {
    ...
    if (Kareem && Kareem.skipWrappedFunction && 
      err instanceof Kareem.skipWrappedFunction) {
      res = err.args[0];
    } else {
      error = err;
    }
 ...
 *
 * @param {Object} fields
 * @see https://github.com/Automattic/orcoos/issues/1091
 ...
 * #### Valid options
 *
 *   - `transform`: optional function which accepts a orcoos document. The return value of the function will be emitted on `data` and returned by `.next()`.
 *
 * @return {QueryCursor}
 * @param {Object} [options]
 ...
 * #### Note:
 *
 * In Mongoose 3.7, `intersects` changed from a getter to a function. If you need the old syntax, use [this](https://github.com/ebensing/orcoos-within).
 *
 * @method intersects
 * @memberOf Query
 * @instance
...
/**
 * this is needed to support the orcoos syntax of:
