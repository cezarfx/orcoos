...
Document.prototype.$session = function $session(session) {
  ...
  if (session != null && session.hasEnded) {
    throw new OrcoosError('Cannot set a document\'s session to a session that has ended. Make sure you haven\'t ' +
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (pathType === 'nested' && val) {
    if (typeof val === 'object' && val != null) {
      ...
      if (val == null) {
        this.invalidate(path, new OrcoosError.CastError('Object', val, path));
        return this;
      }
  ...
  try {
    ...
    if (e instanceof OrcoosError.StrictModeError && e.isImmutableError) {
      this.invalidate(path, e);
    } else if (e instanceof OrcoosError.CastError) {
      this.invalidate(e.path, e);
      if (e.$originalErrorPath) {
        this.invalidate(path,
          new OrcoosError.CastError(schema.instance, val, path, e.$originalErrorPath));
      }
    } else {
      this.invalidate(path,
        new OrcoosError.CastError(schema.instance, val, path, e));
    }
    shouldSet = false;
  }
...
Document.prototype.$inc = function $inc(path, val) {
  ...
  if (schemaType == null) {
    ...
    this.invalidate(path, new OrcoosError.CastError(schemaType.instance, val, path));
    return this;
  }
  ...
  try {
    ...
    this.invalidate(path, new OrcoosError.CastError('number', val, path, err));
  }
...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  if (typeof pathsToValidate === 'function' || typeof options === 'function' || typeof arguments[2] === 'function') {
    throw new OrcoosError('Document.prototype.validate() no longer accepts a callback');
  }
    ...
    if (validationError) {
      for (const key in validationError.errors) {
        ...
        if (!this[documentArrayParent] &&
            validationError.errors[key] instanceof OrcoosError.CastError) {
...
Document.prototype.validateSync = function(pathsToValidate, options) {
  ...
  if (err) {
    for (const key in err.errors) {
      // Make sure cast errors persist
      if (err.errors[key] instanceof OrcoosError.CastError) {
...
Document.prototype.populate = async function populate() {
  ...
  if (typeof args[args.length - 1] === 'function') {
    throw new OrcoosError('Document.prototype.populate() no longer accepts a callback');
  }
...
Document.prototype.$assertPopulated = function $assertPopulated(path, values) {
  ...
  if (!this.$populated(path)) {
    throw new OrcoosError(`Expected path "${path}" to be populated`);
  }
