...
function castPipelineOperator(op, val) {
  if (op === '$unset') {
    if (typeof val !== 'string' && (!Array.isArray(val) || val.find(v => typeof v !== 'string'))) {
      throw new OrcoosError('Invalid $unset in pipeline, must be ' +
  ...
  if (op === '$project') {
    if (val == null || typeof val !== 'object') {
      throw new OrcoosError('Invalid $project in pipeline, must be an object');
    }
    return val;
  }
  if (op === '$addFields' || op === '$set') {
    if (val == null || typeof val !== 'object') {
      throw new OrcoosError('Invalid ' + op + ' in pipeline, must be an object');
    }
    return val;
  ...
  throw new OrcoosError('Invalid update pipeline operator: "' + op + '"');
}
