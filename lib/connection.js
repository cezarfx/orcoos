...
const OrcoosClient = require("./nosqldb-adapter/client").OrcoosClient;
...
Connection.prototype.createCollection = async function createCollection(collection, options) {
  if (typeof options === 'function' || (arguments.length >= 3 && typeof arguments[2] === 'function')) {
    throw new OrcoosError('Connection.prototype.createCollection() no longer accepts a callback');
  }
...
Connection.prototype.startSession = async function startSession(options) {
  if (arguments.length >= 2 && typeof arguments[1] === 'function') {
    throw new OrcoosError('Connection.prototype.startSession() no longer accepts a callback');
  }
...
Connection.prototype.dropCollection = async function dropCollection(collection) {
  if (arguments.length >= 2 && typeof arguments[1] === 'function') {
    throw new OrcoosError('Connection.prototype.dropCollection() no longer accepts a callback');
  }
...
Connection.prototype.dropDatabase = async function dropDatabase() {
  if (arguments.length >= 1 && typeof arguments[0] === 'function') {
    throw new OrcoosError('Connection.prototype.dropDatabase() no longer accepts a callback');
  }
...
function _validateArgs(uri, options, callback) {
  if (typeof options === 'function' && callback == null) {
    throw new OrcoosError('Connection.prototype.openUri() no longer accepts a callback');
...
async function _createMongoClient(conn, uri, options) {
  if (typeof uri !== 'string') {
    throw new OrcoosError('The `uri` parameter to `openUri()` must be a ' +
      `string, got "${typeof uri}". Make sure the first parameter to ` +
      '`mongoose.connect()` or `mongoose.createConnection()` is a string.');
  }

  if (conn._destroyCalled) {
    throw new OrcoosError(
  ...
  if (conn.readyState === STATES.connecting || conn.readyState === STATES.connected) {
    if (conn._connectionString !== uri) {
      throw new OrcoosError('Can\'t call `openUri()` on an active connection with ' +
  ...
  try {
    ...
    client = new OrcoosClient(uri, options);
  } catch (error) {
...
Connection.prototype.destroy = async function destroy(force) {
  if (typeof force === 'function' || (arguments.length === 2 && typeof arguments[1] === 'function')) {
    throw new OrcoosError('Connection.prototype.destroy() no longer accepts a callback');
  }
...
Connection.prototype.close = async function close(force) {
  if (typeof force === 'function' || (arguments.length === 2 && typeof arguments[1] === 'function')) {
    throw new OrcoosError('Connection.prototype.close() no longer accepts a callback');
  }
...
Connection.prototype.model = function(name, schema, collection, options) {
  if (!(this instanceof Connection)) {
    throw new OrcoosError('`connection.model()` should not be run with ' +
  ...
  if (this.models[name] && !collection && opts.overwriteModels !== true) {
    ...
    if (schema && schema.instanceOfSchema && schema !== this.models[name].schema) {
      throw new OrcoosError.OverwriteModelError(name);
    }
...
Connection.prototype.setClient = function setClient(client) {
  ...
  if (!(client instanceof OrcoosClient)) {
    //throw new OrcoosError('Must call `setClient()` with an instance of MongoClient');
    throw new OrcoosError('Must call `setClient()` with an instance of OrcoosClient');
  }
  if (this.readyState !== STATES.disconnected) {
    throw new OrcoosError('Cannot call `setClient()` on a connection that is already connected.');
  }
  if (client.topology == null) {
    throw new OrcoosError('Cannot call `setClient()` with a MongoClient that you have not called `connect()` on yet.');
  }
