...
class OrcoosClient {
  constructor(uri, options) {
    ...
    // });
    return interceptMethodCalls(this, (m, args) => console.log("___ OrcoosClient." + m + "(" + JSON.stringify(args) +")"));
  }
...
class OrcoosDb {
  ...
  constructor(client, options, dbName) {
    ...
    this.options = options;
    this.dbName = dbName;
    return interceptMethodCalls(this, (m, args) => console.log("___ OrcoosDb." + m + "(" + JSON.stringify(args) +")"));
  }
  ...
  createCollection(colName, options) {
    console.log('   o Create table:' + colName);
...
class OrcoosCollection //extends Collection 
{
  constructor(db, colName) {
    ...
    return interceptMethodCalls(this, (m, args) => console.log("___ OrcoosCollection." + m + "(" + JSON.stringify(args) +")"));
  }
  ...
  insertOne(obj, saveOptions) {
    ...
    return this.db.client.put(this.colName, row)
      .then((r) => {
        ...
        return {acknoledged: true, insertedId: kvid};
      });
  }
  ...
  find(filter, options) {
    console.log("    o find " + this.colName + " : " + JSON.stringify(filter));

    let stmt = 'select kvjson  from ' + this.colName;
    return this._execQuery(stmt);
  }

  async _execQuery(stmt, maxLimit = 200) {
    return new Promise( async function (resolve, reject) {
        try {
            let gen = this.client.queryIterable(stmt);
            let rows = [];
            let count = 0;
...
Connection.prototype.setClient = function setClient(client) {
  ...
  if (!(client instanceof OrcoosClient)) {
    throw new MongooseError('Must call `setClient()` with an instance of OrcoosClient');
  }
