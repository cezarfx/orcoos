...
const Collection = require('../../nosqldb-adapter/client').OrcoosCollection;
...
const syncCollectionMethods = { watch: true, find: true, aggregate: true, nosqlQuery: true };
    ...
    try {
      if (collection == null) {
        const message = 'Cannot call `' + this.name + '.' + i + '()` before initial connection ' +
          'is complete if `bufferCommands = false`. Make sure you `await orcoos.connect()` if ' +
...
for (const key of Object.getOwnPropertyNames(Collection.prototype)) {
  // Janky hack to work around gh-3005 until we can get rid of the orcoos
