...
const ondb = require("../../nosqldb-adapter/client");
...
NativeConnection.prototype.createClient = async function createClient(uri, options) {
  ...
  let client;
  try {
    client = new ondb.OrcoosClient(uri, options);
  } catch (error) {
...
NativeConnection.prototype.setClient = function setClient(client) {
  if (!(client instanceof ondb.OrcoosClient)) {
    throw new MongooseError('Must call `setClient()` with an instance of NoSQLClient');
  }
