 ...
 */
Model.nosqlQuery = function nosqlQuery(statement, options = {}) {
  _checkContext(this, 'nosqlQuery');
  
  const mq = new this.Query({}, {}, this, this.$__collection);
  return mq.nosqlQuery(statement, options);
};
  ...
  if (Array.isArray(doc)) {
    ...
    if (args.length === 2 &&
      ...
      utils.warn('WARNING: to pass a `session` to `Model.create()` in ' +
        'OndbMongoose, you **must** pass an array as the first argument. See: ' +
...
/**
 * Gets table limits of throughput and storage for the table associated with this Model.
 ...
 * @returns {import('oracle-nosqldb').TableLimits} a Promice of the limits.
 */
Model.getLimits = async function() {
    let tableName = this.collection.collectionName;
    let table = await this.db.db.client.getTable(tableName);
    return table?.tableLimits;
}

/**
 * Sets new limits of throughput and storage for the table associted with this Model.
 * This method excutes only when connected to a Cloud Service or Cloud Simulator
 * otherwise it's ignored and returns undefined.
 * @param {import('oracle-nosqldb').TableLimits} limits new table limits to be set. [More datails](https://oracle.github.io/nosql-node-sdk/interfaces/TableLimits.html).
 * @returns {import('oracle-nosqldb').TableLimits} a Promice of the updated limits.
 */
Model.setLimits = async function(limits) {
    let isCloud = this.db?.db?.orcoosClient?.connectionString?.getConnectionType() === 'cloud';
    if (!isCloud) {
        return undefined;
    }
    
    let tableName = this.collection?.collectionName;
    let tr = await this.db?.db?.client?.setTableLimits(tableName, limits);
    return tr?.tableLimits;
}
