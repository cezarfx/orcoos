 ...
 * 
 * - nosqldb[+service_type][+protocol] - the scheme of the connection string, case insensitive.
 ...
 * - host1[:port1][, ...hostN[:portN]] - hostnames (case insensitive) and their port numbers, at least one hostname is required, depending on the service type (see below) these values have the following semantics:
        ...
        try {
            ...
            if (!this.isServiceTypeExplicit()) {
                ...
                try {
                    ...
                    throw new Error('Could not connect to: "' + this._connectionString + '" tried both cloud and on-prem.');
                }
            } else {
                throw new Error('Could not connect to: "' + this._connectionString + '"');
            }
        }
        return client;
    }
    ...
    getClientConfig(connectOptions) {
        ...
        if (connectOptions) {
            if (connectOptions.dbName) {
                if (this.serviceType === SERVICE_CLOUD) {
