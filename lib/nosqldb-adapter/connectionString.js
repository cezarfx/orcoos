...
const OPTION_CONSISTENCY_EVENTUAL = 'eventual';
...
const OPTION_SECURITY_INFO_TIMEOUT ='securityInfoTimeout';
...
class NoSQLConnectionString {
    ...
    async getClient(options) {
        ...
        // Check connection
        try {
            if (this._connectionType === CONNECTION_ON_PREM) {
                // this is a system op not allowed on cloud/cloudsim
    ...
    getClientConfig(connectOptions = {}) {
        ...
        if (this._options.get(OPTION_SECURITY_INFO_TIMEOUT)) {
            config.securityInfoTimeout = this._options.get(OPTION_SECURITY_INFO_TIMEOUT);
        }
        if (this._options.get(OPTION_TABLE_POLL_DELAY)) {
            config.tablePollDelay = this._options.get(OPTION_TABLE_POLL_DELAY);
        }
        if (this._options.get(OPTION_TABLE_POLL_TIMEOUT)) {
            config.tablePollTimeout = this._options.get(OPTION_TABLE_POLL_TIMEOUT);
        }
        if (this._options.get(OPTION_TIMEOUT)) {
            config.timeout  = this._options.get(OPTION_TIMEOUT);
        }
        ...
        if (connectOptions) {            
            ...
            if (connectOptions.maxTimeMS) {
                config.timeout = connectOptions.maxTimeMS;
            }

            // connectOptions.readConcern only EVENTUAL is supported anything else is left to NoSQL DB options
            if (connectOptions.readConcern && connectOptions.readConcern.toLowerCase() === OPTION_CONSISTENCY_EVENTUAL) {
                config.consistency = OPTION_CONSISTENCY_EVENTUAL;
            }

            // connectOptions.writeConcern.w: 'MAJORITY' is supported as config.durability.replicaAck = 'SIMPLE_MAJORITY', any other value is an error
            // connectOptions.writeConcern.j: true is config.durability.masterSync = 'SYNC'
            // connectOptions.writeConcern.j: false is config.durability.masterSync = 'NO_SYNC'
            if (connectOptions.writeConcern) {
                if (connectOptions.writeConcern.w) {
                    if (typeof connectOptions.writeConcern.w === 'string' &&
                        connectOptions.writeConcern.w.toLowerCase() === 'majority') {
                        config.durability.replicaAck = 'SIMPLE_MAJORITY';
                    } else {
                        throw new Error('Unsupported writeConcern.w connection option:' + 
                            connectOptions.writeConcern.w + 
                            '. Only w: "majority" is supported as durability.replicaAck: "SIMPLE_MAJORITY".');
                    }
                    if (connectOptions.writeConcern.j === true || 
                        ( typeof connectOptions.writeConcern.j === 'string' && 
                        connectOptions.writeConcern.j.toLowerCase() === 'true')) {
                        config.durability.masterSync = 'SYNC';
                    } else if (connectOptions.writeConcern.j === false || 
                        ( typeof connectOptions.writeConcern.j === 'string' && 
                        connectOptions.writeConcern.j.toLowerCase() === 'false')) {
                        config.durability.masterSync = 'NO_SYNC';
                    } else {
                        throw new Error('Unsupported writeConcern.j option:' + 
                            connectOptions.writeConcern.j + 
                            '. Only true or false is supported as durability.masterSync: "SYNC"/"NO_SYNC".');
                    }
                }
            }
            ...
            if (connectOptions[OPTION_SECURITY_INFO_TIMEOUT]) {
                config.securityInfoTimeout = connectOptions[OPTION_SECURITY_INFO_TIMEOUT];
            }
            if (connectOptions[OPTION_TABLE_POLL_DELAY]) {
                config.tablePollDelay = connectOptions[OPTION_TABLE_POLL_DELAY];
            }
            if (connectOptions[OPTION_TABLE_POLL_TIMEOUT]) {
                config.tablePollTimeout = connectOptions[OPTION_TABLE_POLL_TIMEOUT];
            }
            if (connectOptions[OPTION_TIMEOUT]) {
                config.timeout = connectOptions[OPTION_TIMEOUT];
            }
        }
    ...
    /**
     * Returns the kind of connection specified or infered: {@link CONNECTION_CLOUD} or {@link CONNECTION_ON_PREM}.
     */
    getConnectionType() {
    ...
    /**
     * Returns true if {@link getConnectionType()} is {@link CONNECTION_CLOUD}.
     */
    isCloud() {
        return this._connectionType === CONNECTION_CLOUD;
    }

    /**
     * Returns true if {@link getConnectionType()} is {@link CONNECTION_ON_PREM}.
     */
    isOnPrem() {
        return this._connectionType === CONNECTION_ON_PREM;
    }
...
module.exports.OPTION_RETRY_SECURITY_INFO_TIMEOUT = OPTION_SECURITY_INFO_TIMEOUT;
