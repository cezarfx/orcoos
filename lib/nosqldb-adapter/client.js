...
class OrcoosDb {
    ...
    //     let table = this.client.getTable(colName);
    //     return table.then(table => {
    //         if (table && table.tableState == nosqldb.TableState.ACTIVE) {
...
class OrcoosCollection //extends Collection 
{
    ...
    // MongoDB accepts ObjectIdinside doc to be inserted in DB
    ...
    async insertMany(objects, options) {
        ...
        for (let obj of objects) {
            await this.insertOne(obj, options);
        }
    }
}
