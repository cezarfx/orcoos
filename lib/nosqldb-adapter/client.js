...
class OrcoosCollection {
    ...
    async createTable(options) {
        ...
        try {
            ...
            let compartment = options?._schema?.options?.collectionOptions?.compartment;
            ...
            let namespace = options?._schema?.options?.collectionOptions?.namespace;
            let tableLimits = options?._schema?.options?.collectionOptions?.tableLimits;
            let timeout = options?._schema?.options?.collectionOptions?.timeout;
            ...
            try {
                let table = await this.db.client.getTable(tableName, JSON.parse(JSON.stringify(this._options)));
                if (table && table.tableState == TableState.ACTIVE) {
                    this.log(LOG_LEVELS.FINNER, "     o OCol.createTable() Table " + (this._options?.namespace ? this._options.namespace + ':': '') + tableName + " already exists.");
                    this._created = true;
            ...
            if (options && options._schema && typeof options._schema?.obj._id === 'object' && 
                ...
                for (let key of keys) {
                    ...
                    if (nonKeyFields.includes(key)) {
                        throw new OrcoosError('Composite key and non-key field name collision: "' + key + '"');
                    }
                }
                ...
                // this.log(LOG_LEVELS.INFO, "Standard one component key _id detected.");
            }

            try {
                this.log(LOG_LEVELS.FINE , 'DDL: ' + stmt);
                let result = await this.db.client.tableDDL(stmt, JSON.parse(JSON.stringify(this._options)));
    ...
    async findOneAndDelete(filter, options) {
        ...
        if (filter && filter._id) {
            ...
            return {value: (resultObj && resultObj[0] ? resultObj[0] : null)};
        }
    ...
    async nosqlQuery(statement, options) {
        ...
        await this._checkTableIsCreated(options); 
        return new OrcoosFindCursor(this, statement);
    }
    ...
    async find(filter, options, schema) {
        ...
        this.log(LOG_LEVELS.INFO, "      o Q: " + stmt);
        await this._checkTableIsCreated(options);
        return new OrcoosFindCursor(this, stmt, bindings);
    }
    ...
    _computeWhere(query, bindings) {
        ...
        if (!(query instanceof Object)) {
            this.log(LOG_LEVELS.SEVERE, 'o Error: Unexpected input value for where expression: ' + JSON.stringify(query));
...
class OrcoosFindCursor {
    ...
    async toArray() {
        ...
        try {
            ...
            if (!prepStmt) {
                prepStmt = await this.collection.db.client.prepare(this.statement, JSON.parse(JSON.stringify(this.collection._options)));
            ...
            this.gen = this.collection.db.client.queryIterable(prepStmt, this.collection._options);
