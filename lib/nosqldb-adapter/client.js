...
class OrcoosCollection //extends Collection 
{
        ...
        for (const prop in compObj) {
            ...
            if (prop.startsWith("$")) {
                ...
                case "$nor":
                    ...
                    break;
                default: 
                    throw Error("Unknown top label operator: " + prop);
                }
    ...
    async createIndex(keys, options, schema) {
        ...
        for (const [key, value] of Object.entries(keys)) {
            ...
            //let dbProp = this._computeDbProp(key).substring(2);  // drop the 't.'
            // let schemaPropType = schema.path(key);
            // if (!schemaPropType || !schemaPropType.instance) {
            //     throw Error("No property named: " + key + " found for " + this.colName);
            // }
            // let [isArray, dbPropType] = this._computeDbType(schemaPropType);
            // if (!isArray) {
            //     // dbProps end in '[]' so that it's transformed in a sequence
            //     // but when defining indexes we need to drop the '[]' is prop is not an array
            //     dbProp = dbProp.substring(0, dbProp.length - 2);
            // }
