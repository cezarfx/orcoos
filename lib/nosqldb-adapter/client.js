...
class OrcoosClient {
    /**
     * This gets called during the initial OndbMongoose connect(connection_string, 
     * options).
     * 
...
class OrcoosCollection {
    ...
    async findOneAndUpdate(filter, update, options) {
        if (filter && filter._id) {
            ...
            return res;
            //return new OrcoosUpdateQuery(this, stmt, options);
        }
    ...
    async createIndex(keys, options, schema) {
        ...
        for (const [key, value] of Object.entries(keys)) {
            if (key.includes('*') || key.includes('$')) {
                throw Error("OndbMongoose does not support wildcard indexes, received: " + key);
            }
            if (value != 1) {
                throw Error("OndbMongoose supports indexes only on path values equal to 1, received: " + value);
            }
