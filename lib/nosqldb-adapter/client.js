...
const { OrcoosMutex } = require('./utils');
const { ObjectId } = require('bson');
const { TableState, NoSQLError, ErrorCode } = require("oracle-nosqldb");
const { LRUCache } = require('lru-cache');
//const { options } = require('..');
// const { join } = require('../helpers/query/validOps');
// const { size } = require('../helpers/specialProperties');
// const { value } = require('../options/propertyOptions');

const USE_COLLECTION_TABLES = true;
...
class OrcoosClient {
    ...
    setMaxListeners(max) { }
    async connect() {
        this.client = await (new NoSQLConnectionString(this.uri)).getClient(this.options);
    }
...
class OrcoosCollection {
        ...
        try {
            ...
            try {
                ...
                if (table && table.tableState == TableState.ACTIVE) {
                    ...
                    this._created = true;
                    let pkList = JSON.parse(table.schema).primaryKey;
                ...
                if (!(e instanceof NoSQLError && e.errorCode === ErrorCode.TABLE_NOT_FOUND)) {
                    this.log(LOG_LEVELS.SEVEREE, "client.getTable error: " + e);
                    throw e;
                }
            }

            let stmt = USE_COLLECTION_TABLES ? 
                'CREATE TABLE IF NOT EXISTS ' + tableName + '(' + KVID + 
                    ' STRING, PRIMARY KEY(' + KVID + ')) AS JSON COLLECTION' :
                'CREATE TABLE IF NOT EXISTS ' + tableName + '(' + KVID + 
                    ' STRING, ' + KVJSON + ' JSON, PRIMARY KEY(' + KVID + '))';
            ...
            if (options && options._schema && typeof options._schema?.obj._id === 'object' && 
                ...
                let nonKeyFields = Object.keys(options._schema.obj).filter(v => v !== '_id');
                ...
                for (let key of keys) {
                    if (typeof options._schema.obj._id[key] === 'function' &&  
                        ...
                        // if (key === KVJSON) {
                        //     throw new OrcoosError('Composite key not allowed to contain field called: ' + key);
                        // }
                        ...
                        keySchema.push({name: key, type: keyType});
                    } else {
                        allKeyTypesAreValid = false;
                        break;
                    }
                    // check if any key field colides with non-key fields
                    if (nonKeyFields.includes(key)) {
                        throw new OrcoosError('Composite key and non-key field names collision: "' + key + '"');
                    }
                }
                ...
                if (allKeyTypesAreValid) {
                    ...
                    this._pkList = keySchema.map(v => v.name);
                    // this.log(LOG_LEVELS.FINE , 'Composite key detected: "' + keyDef + '" for table: ' + tableName);
                    // stmt = 'CREATE TABLE IF NOT EXISTS ' + tableName + '(' + keyDef + KVJSON + ' JSON, PRIMARY KEY(' + this._pkList.join(', ') + '))';
                    stmt = USE_COLLECTION_TABLES ? 
                        'CREATE TABLE IF NOT EXISTS ' + tableName + '(' + keyDef + 
                            ' PRIMARY KEY(' + this._pkList.join(', ') + 
                            ')) AS JSON COLLECTION' :
                        'CREATE TABLE IF NOT EXISTS ' + tableName + '(' + keyDef +
                             KVJSON + ' JSON, PRIMARY KEY(' + 
                             this._pkList.join(', ') + '))';
                } else {
                    throw new OrcoosError('Invalid composite key for table: ' + tableName);
                }
            } else {
                // this.log(LOG_LEVELS.INFO, "No composite key!!!");
            }
    ...
    async findOne(conditions, options) {
        ...
        if (conditions._id) {
            ...
            return this.db.client.get(this.colName, row)
                .then((r) => {
    ...
    async deleteOne(where, options) {
        ...
        if (where._id) {
            ...
            return this.db.client.delete(this.colName, pk)
                .then((r, e) => r.success);
        }
    ...
    async _queryPromise(client, stmt, bindings, options, maxLimit = MAX_QUERY_RESULTS_LIMIT) {        
        try {
            ...
            if (!prepStmt) {
                prepStmt = await this.db.client.prepare(stmt);
            ...
            let gen = client.queryIterable(prepStmt);
            let rows = [];
            let count = 0;
    ...
    async replaceOne(filter, replacement, options) {
        ...
        row[KVJSON] = OrcoosCollection._fixTypesBeforeInsert(replacement);
        row[KVJSON]._id = row[KVID];
        await this._checkTableIsCreated(options);
        ...
        //     });
        this.log(LOG_LEVELS.INFO, '    o > row: ' + JSON.stringify(row));
        let putRes = await this.db.client.putIfPresent(this.colName, row);
        this.log(LOG_LEVELS.INFO, '    o > putIfPresent: ' + JSON.stringify(putRes.success));
    ...
    async nosqlQuery(statement, options) {
        ...
        await this._checkTableIsCreated(options); //todo pass options here
    ...
    async find(filter, options, schema) {
        ...
        this.log(LOG_LEVELS.INFO, "      o Q: " + stmt);
        await this._checkTableIsCreated(options);
        return new OrcoosFindCursor(this.db, stmt, bindings);
    }
    
    _computeWhere(query, bindings) {
        ...
        if (query._id && (typeof query._id === "string" || 
            ...
            query._id instanceof Object
        )) {
            let whereExp = this._computeWherePk(bindings, query._id);
            return whereExp?.length > 0 ? ' WHERE (' + this._computeWherePk(bindings, query._id) + ')' : '';
        } else {
            let cond = this._computeCompStartExp(query, bindings);
            if (cond == "") {
                return "";
            }
            return ' WHERE ' + cond;
...
class OrcoosFindCursor {
    constructor(db, statement, bindings, maxLimit = MAX_QUERY_RESULTS_LIMIT) {
        this.db = db;
    ...
    async toArray() {
        ...
        let count = 0;
        try {
            let prepStmt = this.db.prepStmtCache.get(this.statement);
            if (!prepStmt) {
                prepStmt = await this.db.client.prepare(this.statement);
                this.db.prepStmtCache.set(this.statement, prepStmt);
            }
            ...
            this.gen = this.db.client.queryIterable(prepStmt);
